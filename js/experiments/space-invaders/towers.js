window.addEventListener("load", function (event) {
  ...
  for (let offsetX = 70; offsetX <= 670; offsetX = offsetX + 200) {
    let tower = [];

    for (let x = 0; x < 60; x = x + 20) {
      for (let y = 0; y < 60; y = y + 20) {
        let towerBlock = {
          x: x + offsetX,
          y: y + offsetY,
          width: 20,
          ...
          cannonLaserPulseCollision: function (laserPulse) {
            if (this.alive) {
              // check overlpa with laserPulse
              if (
                laserPulse.y > this.y &&
                laserPulse.y < this.y + this.height &&
                laserPulse.x > this.x &&
          ...
          spacecraftLaserPulseCollision: function (laserPulse) {
            // only detect collision if invader is alive
            if (this.alive) {
              // check overlpa with laserPulse
              if (
  ...
  function draw() {
    ...
    for (let tower of towers) {
      for (let t of tower) {
        t.draw();
        t.cannonLaserPulseCollision(laserPulse);
        t.spacecraftLaserPulseCollision(spacecraftLaserPulse);
      }
    }
